#!/usr/bin/env bash
# 最小代价更新订阅：宿主机经 127.0.0.1:SUBCONVERTER_HOST_PORT 访问 subconverter，拉取 YAML 写入容器后 PUT /configs 重载，不重启容器。
# 控制面端口、混合代理宿主机端口、subconverter 宿主机端口由 clash-env.inc.sh 从 .env / .env.example 解析。
# 用法: ./clash-subscription-hot-reload.sh
# 失败时可改用: ./clash-subscription-rebuild.sh 重建容器
...
if ! curl -s -o /dev/null -w "%{http_code}" --connect-timeout 3 "http://127.0.0.1:${SUBCONVERTER_HOST_PORT}/version" 2>/dev/null | grep -q '200'; then
  echo "无法访问 subconverter (127.0.0.1:${SUBCONVERTER_HOST_PORT})，请确认容器已启动。"
  echo "可尝试: ./clash-subscription-rebuild.sh 重建容器"
  exit 1
fi
...
if ! curl -s -o "$TMP_CONFIG" --connect-timeout 30 "http://127.0.0.1:${SUBCONVERTER_HOST_PORT}/sub?target=clash&url=${ENCODED_URL}"; then
  echo "拉取订阅失败。可尝试: ./clash-subscription-rebuild.sh 重建容器"
  exit 1
fi
if [ ! -s "$TMP_CONFIG" ]; then
  echo "拉取结果为空。可尝试: ./clash-subscription-rebuild.sh 重建容器"
  exit 1
fi

echo "正在写入容器并重载 Clash..."
if ! docker cp "$TMP_CONFIG" clash-with-ui:"${CLASH_CONFIG_PATH}" 2>/dev/null; then
  echo "写入容器失败，请确认 clash-with-ui 容器在运行。可尝试: ./clash-subscription-rebuild.sh 重建容器"
  exit 1
fi
...
if [ "$code" != "204" ] && [ "$code" != "200" ]; then
  echo "Clash 重载失败 (PUT /configs, HTTP ${code})。可尝试: ./clash-subscription-rebuild.sh 重建容器"
  exit 1
fi

echo "订阅已更新，Clash 已重载（无重启）。"
echo ""
echo "--- 更新后验证（经容器代理 ${ALL_PROXY_PORT} 访问 ip-api.com）---"
if curl -x "http://127.0.0.1:${ALL_PROXY_PORT}" -s --connect-timeout 10 "http://ip-api.com/json/" 2>/dev/null | head -20; then
  echo "---"
  echo "容器代理可用。可再执行 ./clash-verify-mixed-proxy-portmap.sh ${ALL_PROXY_PORT} 做完整验证。"
else
  echo "（经代理请求未返回，请检查端口或执行 ./clash-verify-mixed-proxy-portmap.sh ${ALL_PROXY_PORT}）"
fi
